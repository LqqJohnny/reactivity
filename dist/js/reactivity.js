...
/******/ 	var hotCurrentHash = "2f39b7f1fab3111b3c66"; // eslint-disable-line no-unused-vars
...
var Node = function () {
  ...
  _createClass(Node, [{
    ...
    value: function createVirtualDom(children) {
      var vnode_children = children.map(function (child) {
    ...
    value: function render() {
      ...
      if (!document) {
        ...
        // 先创建一个最外层的父元素
        var fatherEle = document.createElement(this.tag);
        if (this.props) {
          Object.keys(this.props).forEach(function (key) {
            fatherEle.setAttribute(key, _this.props[key]);
          });
        }
    ...
    key: "appendChildNode",
    value: function appendChildNode(el, children) {
      var _this2 = this;
      ...
      children.forEach(function (child) {
        ...
        if (child.tag === "string") {
          ...
          if (child.children instanceof Array && child.children.length > 0) {
            childEl = _this2.appendChildNode(childEl, child.children);
          }
        }
...
var _dom2vdom = __webpack_require__(4);
...
var Reactivity = function () {
  function Reactivity(el, data) {
    ...
    this.templateDom = el;
    this.proxy(data);
    (0, _observe.observe)(this._data, function (key, val) {
      _this.srcData[key] = val;
      _this.preVdom = _this.appVdom; //  保存上一次的vdom
      _this.updatePage(); // patch 表示变化的数据
    });
    this.render(); // 初始化页面
  }

  _createClass(Reactivity, [{
    key: 'render',
    value: function render() {
      // 从app的第一个非text的 dom子元素 开始
      var appChild = Array.from(this.el.childNodes).filter(function (val) {
        return val.nodeName !== '#text';
      })[0];

      var appJson = (0, _dom2vdom.dom2Vdom)(appChild, this.srcData);
      this.templateJson = JSON.parse(JSON.stringify(appJson)); // 先保存最初的模板格式
      var appVdom = new _node2.default(this.replaceKey(appJson, this.srcData));
      this.appVdom = appVdom;

      // 得到 所有 vdom 之后 ，清除 appChild
      this.el.innerHTML = "";
      // 初始渲染页面
      (0, _diff.diff)(this.el, appVdom);
    }
  }, {
    key: 'proxy',
    ...
    value: function updatePage() {
      ...
      var appVdom = new _node2.default(this.replaceKey(this.templateJson, this.srcData));
      this.appVdom = appVdom;
      (0, _diff.diff)(this.el, appVdom, this.preVdom);
    }
  }, {
    key: 'replaceKey',
    value: function replaceKey(json, data) {
      var _this2 = this;

      if (json.tag === "string") {
        json.value = (0, _dom2vdom.replaceData)(json.value, data);
        return json;
      } else {
        // 是 dom 节点 可能有children
        if (json.children && json.children.length > 0) {
          json.children = json.children.map(function (child) {
            return _this2.replaceKey(child, data);
          });
        }
      }
      return json;
...
function diff($parent, newNode, oldNode) {
  ...
  if (!oldNode) {
    ...
    if (!newNode.children && !oldNode.children) {
      ...
      var newLen = Math.max(newNode.children.length, oldNode.children.length);
...
function isChanged(node1, node2) {
  return (typeof node1 === 'undefined' ? 'undefined' : _typeof(node1)) !== (typeof node2 === 'undefined' ? 'undefined' : _typeof(node2)) || typeof node1 === 'string' && node1 !== node2 || node1.tag !== node2.tag;
}
...
exports.defineReactivity = defineReactivity;
 ...
 */
function defineReactivity(obj, key, value, cb) {
...
exports.dom2Vdom = dom2Vdom;
exports.getProps = getProps;
exports.replaceData = replaceData;
function dom2Vdom(el, data) {
  ...
  var json = {
    ...
    var children = Array.from(el.childNodes).map(function (val) {
...
function getProps(el) {
  ...
  Array.from(el.attributes).forEach(function (val) {
    props[val.name] = val.value;
  });
  return props;
}

function replaceData(srcText, srcData) {
